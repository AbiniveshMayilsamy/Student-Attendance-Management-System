...
export default function TeacherDashboard() {
  const { user } = useAuth();
  const { classes, subjects, attendance, students, fetchAttendance } = useData();

  // subjects populated with teacherId as object from API
  ...
  const myClasses = classes.filter(c => myClassIds.includes(c._id));

  const today = new Date().toISOString().split('T')[0];

  useEffect(() => {
    if (myClassIds.length) fetchAttendance({ date: today });
  }, [subjects]);

  const todayRecords = attendance.filter(r => r.date === today && myClassIds.includes(r.classId?._id || r.classId));
  const presentToday = todayRecords.filter(r => r.status === 'present' || r.status === 'late').length;
  const recent = [...attendance].sort((a, b) => b.date.localeCompare(a.date)).slice(0, 6);
        ...
        <div className="bg-white/5 backdrop-blur-md border border-white/10 rounded-xl p-5">
          ...
          {myClasses.length === 0 ? <p className="text-sm text-white/30">No classes assigned</p> : (
            <div className="space-y-2">
              {myClasses.map(c => (
                <div key={c._id} className="flex items-center justify-between p-3 bg-white/5 border border-white/10 rounded-lg">
                  <div>
          ...
          <Table headers={['Student', 'Date', 'Status']}>
            {recent.map(r => (
              <tr key={r._id} className="hover:bg-white/5">
                <td className="px-4 py-2.5 text-sm text-white/70">{r.studentId?.name || '—'}</td>
