...
function StudentForm({ initial, onSave, onClose, classes }) {
  ...
  return (
    <form onSubmit={e => { e.preventDefault(); onSave(form); }} className="space-y-4">
      ...
      <Select label="Class" required value={form.classId?._id || form.classId} onChange={e => set('classId', e.target.value)}>
...
export default function StudentsList() {
  ...
  const filtered = students.filter(s => {
    ...
    const matchClass  = !filterClass || (s.classId?._id || s.classId) === filterClass;
    return matchSearch && matchClass;
  });

  const handleSave = async (data) => {
    if (modal === 'add') await addStudent(data);
    else await updateStudent(modal._id, data);
    setModal(null);
  };
          ...
          return (
            <tr key={s._id} className="hover:bg-white/5">
              ...
              <td className="px-4 py-3">
                <Link to={`/students/${s._id}`} className="text-sm font-medium text-indigo-300 hover:text-indigo-200 hover:underline">{s.name}</Link>
              </td>
              <td className="px-4 py-3 text-sm text-white/60">{cls?.name || s.classId?.name || '—'}</td>
              ...
              <td className="px-4 py-3">
                <div className="flex gap-2">
                  ...
                  <Btn size="sm" variant="danger"  onClick={() => deleteStudent(s._id)}>Delete</Btn>
                </div>
              </td>
            </tr>
          );
